    ...
    <div className="flex-1 flex flex-col min-h-screen bg-brand-bg text-neutral-900 font-sans select-none">
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-neutral-200/80 px-4 sm:px-6 py-3 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-2">
          <div className="w-9 h-9 bg-brand-primary rounded-xl flex items-center justify-center text-white shadow-md shadow-brand-primary/25 transform rotate-0 hover:rotate-0 transition-transform">
            <span className="font-extrabold text-lg">CD</span>
          </div>
          <div>
            <h1 className="text-sm font-black tracking-tight text-neutral-900 leading-none">
              CHICKEN DELIGHT
            </h1>
            <p className="text-[9px] font-bold text-brand-primary tracking-widest mt-0.5">
              ONLINE ORDERING
            </p>
          </div>
        </div>
      ...
      <div className="md:hidden flex items-center justify-between px-4 py-2.5 bg-neutral-100 border-b border-neutral-200 text-xs">
        <div className="flex items-center gap-2">
          <button 
            onClick={() => setOrderType(orderType === 'takeout' ? 'delivery' : 'takeout')}
            className="bg-white border border-neutral-200 px-2.5 py-1 rounded-lg font-bold text-neutral-700 capitalize"
          >
            {orderType}
          </button>
          <button
            onClick={() => setShowAddressModal(true)}
            className="flex items-center gap-1 text-neutral-600 font-medium truncate max-w-[180px]"
          >
            <MapPin size={11} className="text-brand-primary flex-shrink-0" />
            <span className="truncate">{address ? address : 'Add Address'}</span>
          </button>
        </div>
        <button
          onClick={() => setShowAddressModal(true)}
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-neutral-500 font-semibold">11:00 AM - 10:00 PM</span>
            </div>
      ...
      <div className="flex-1 flex overflow-hidden p-4 sm:p-6 gap-6 min-h-0">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {Array.from({ length: 6 }).map((_, idx) => (
                <div key={idx} className="flex flex-col bg-white rounded-2xl border border-neutral-200/60 overflow-hidden p-3.5 gap-3 animate-pulse">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative h-[95px] w-[95px] sm:h-[130px] sm:w-full bg-neutral-50 rounded-xl overflow-hidden flex-shrink-0">
                      <img
                        src={item.image || "https://images.unsplash.com/photo-1569058242253-92a9c755a0ec?w=300&auto=format&fit=crop&q=60"}
                        alt={item.name}
                        onError={(e) => {
                          (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1569058242253-92a9c755a0ec?w=300&auto=format&fit=crop&q=60";
                        }}
                      ...
                      {item.badge && (
                        <span className="absolute top-2 left-2 bg-brand-primary text-white text-[8px] sm:text-[8.5px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-md flex items-center gap-0.5 shadow-sm">
                          <Flame size={9} className="fill-white animate-pulse" />
                          {item.badge}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex-1 flex flex-col justify-between gap-1.5 sm:gap-3 min-w-0">
                      ...
                      <div className="flex justify-between items-center mt-1 border-t border-neutral-50 pt-2 sm:pt-2.5">
                        <div>
                          <p className="text-[8.5px] sm:text-[10px] text-neutral-400 font-bold uppercase tracking-wider">Price</p>
                          <p className="text-[12px] sm:text-[14px] font-black text-neutral-800">${item.price.toFixed(2)}</p>
                        </div>
                        <button
            ...
            <div className="flex flex-col items-center justify-center h-48 border border-dashed border-neutral-300/80 rounded-2xl p-6 text-center bg-white">
              ...
              <h4 className="text-xs font-bold text-neutral-700">No dishes match your filters</h4>
              <p className="text-[10px] text-neutral-400 mt-1 max-w-[240px] leading-relaxed">
                Could not find any items matching &ldquo;{searchQuery}&rdquo; in our {categories.find(c => c.id === selectedCategory)?.name || 'menus'}.
              </p>
            </div>
          )}
        </div>
        ...
        <aside className="hidden lg:flex w-[28%] flex-col bg-white border border-neutral-200/80 rounded-2xl p-5 shadow-sm h-fit max-h-[85vh]">
          ...
          <div className="flex-1 overflow-y-auto min-h-0 py-3 divide-y divide-neutral-100 max-h-[40vh] no-scrollbar pr-0.5">
            {cartItems.length > 0 ? (
                      ...
                      {cartItem.selectedModifiers.length > 0 && (
                        <div className="text-[9px] text-neutral-400 mt-1 space-y-0.5">
                          {cartItem.selectedModifiers.map((mod) => (
                            <p key={mod.optionId} className="flex items-center gap-1 pl-1">
              ...
              <div className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <p className="text-xs font-bold text-neutral-600">Your bag is empty</p>
          ...
          {cartItems.length > 0 && (
            <div className="border-t border-neutral-200/80 pt-3.5 space-y-2 flex-shrink-0">
              ...
              <button
                onClick={() => toast.success('Online checkout coming soon!')}
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-white h-full flex flex-col p-5 shadow-2xl z-10 animate-drawer-slide-in">
            ...
            {/* Cart Items Scroll */}
            <div className="flex-1 overflow-y-auto py-3 divide-y divide-neutral-100 no-scrollbar">
              {cartItems.length > 0 ? (
                cartItems.map((cartItem) => (
                  <div key={cartItem.id} className="py-3 flex flex-col gap-1.5">
                    <div className="flex items-start gap-2 justify-between">
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center">
                  ...
                  <p className="text-[10px] text-neutral-400 max-w-[200px] leading-relaxed mt-1">
                    Select mouthwatering meals from the menu list to build your order!
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="border-t border-neutral-200 pt-3.5 space-y-2 flex-shrink-0">
                ...
                <div className="flex gap-2 pt-3">
                  ...
                  <button
                    onClick={() => {
                      setIsCartOpen(false);
                      toast.success('Online checkout coming soon!');
                    }}
      ...
      {cartItems.length > 0 && !isCartOpen && (
        <div className="lg:hidden fixed bottom-0 left-0 right-0 p-4 bg-white/95 backdrop-blur-md border-t border-neutral-200 shadow-lg z-30 flex items-center justify-between gap-3 animate-fade-in">
          <div>
            <p className="text-[9px] text-neutral-400 font-extrabold uppercase tracking-wide">My Bag Total</p>
            <p className="text-[13px] font-black text-neutral-800">
              ${total.toFixed(2)}&nbsp;
              <span className="text-[10px] text-neutral-400 font-normal">
                ({cartItems.reduce((acc, curr) => acc + curr.quantity, 0)} item{cartItems.length !== 1 ? 's' : ''})
              </span>
            </p>
          </div>
          <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <p className="text-[10px] text-neutral-400 font-medium leading-relaxed">
              Please enter your delivery details below to check availability and calculate delivery fees.
            </p>
